        ...
        <div class="col-md-4 left-block">
            <div class="form-group">
                ...
                <select onchange="dopFilter(event)" id="degreeSelect" class="ajax-filter chsn form-control" name="degree">
            ...
            <div class="form-group">
                ...
                <select onchange="dopFilter(event)" id="oblSelect" class="ajax-filter chsn form-control" name="sphere">
                    <option value="">Выберите</option>
                    @foreach($dirs as $s)
                        <option @if($direction_id??$dir_id == $s->id) selected @endif value="{{$s->id}}">{{$s->name_ru}}</option>
                    @endforeach
                </select>
            </div>
            <div id="dir" class="form-group" style="display: none">
                <label>Направление подготовки</label>
                <select onchange="dopFilter(event)" id="dirSelect" class="ajax-filter chsn form-control" name="direction">
                    <option value="">Выберите</option>
                    @foreach($subDir as $d)
            ...
            <div id="income" class="form-group" style="display: none">
                ...
                <select onchange="dopFilter(event)" id="incomeSelect" class="ajax-filter chsn form-control" name="when">
                    <option value="">Выберите</option>
        ...
        <div class="col-md-8">
            <div class="pagination-block m-2 p-2">
                <div class="row m-1">
                    <div @if($page > 0) onclick="window.location='{{action('PagesController@showDoctor', ['degree' => $degree??'any', 'pages' => ($page-1), 'direction_id' => $dir_id??$direction_id, 'city_id' => $city_id, 'search' => $query])}}'" style="cursor: pointer" @else disabled @endif class="col-1 text-center"><img src="{{asset('img/pagination-left.svg')}}" alt=""></div>
                        ...
                        <div id="select-div">
                            <select id="pagination-select" class="custom-control-inline border-0 m-0" style="outline: 0" onchange="javascript:location.href = this.value;">
                                @for($i = 0; $i < ceil(count($costs)/10); $i++)
                                    ...
                                    @else
                                        <option class="nPage" value="{{action('PagesController@showDoctor', ['degree' => $degree??'any', 'pages' => $i, 'direction_id' => $dir_id??$direction_id, 'city_id' => $city_id, 'search' => $query])}}">Страница {{$i+1}}</option>
                                    @endif
                                @endfor
                            </select>
                    ...
                    <div @if($page < ceil(count($costs)/10)-1) onclick="window.location='{{action('PagesController@showDoctor', ['degree' =>  $degree??'any', 'pages' => ($page+1), 'direction_id' => $dir_id??$direction_id, 'city_id' => $city_id, 'search' => $query])}}'" style="cursor: pointer" @else disabled @endif class="col-1 text-center"><img src="{{asset('img/pagination-right.svg')}}" alt=""></div>
                </div>
            </div>
        </div>
